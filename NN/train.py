...
from NN.nltk_funcs import bag_of_words, stem, tokenize
...
def train():
    ...
    all_words = [
        ...
        for w in all_words
        if w in all_words
        if w not in ignore_punct or stopwords.words("english")
    ]  # Replace the list of all words with only stemmed words that contain value
    ...
    for pattern_sentence, tag in pattern_tags:
        ...
        )  # Create bag of words based off of sentence
    ...
    for epoch in range(num_epochs):
        ...
        for words, labels in train_loader:
            ...
            loss = cross_entropy(
                outputs, labels
            )  # Calculate cross entropy loss ((y_predicted - y)**2).mean(), where y_predicted is the expected output and y is actual output, and softmax of output
